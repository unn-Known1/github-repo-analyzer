...
class RepoAnalyzer:
    ...
    def _calculate_health(self, repo) -> Dict[str, Any]:
        ...
        # Check recent activity (within 90 days default)
        ...
        # Check for CI/CD (simple heuristic: look for common CI files)
        ci_files = ['.travis.yml', '.github/workflows', 'Jenkinsfile', '.circleci', 'azure-pipelines.yml']
        has_ci = any(True for _ in [repo.get_contents(f) for f in ci_files if self._file_exists(repo, f)])
        if has_ci:
    ...
    def _get_community_metrics(self, repo) -> Dict[str, Any]:
        ...
        try:
            ...
            # Stargazers over time (approximate via current/fork ratio)
...
Examples:
  ...
  %(prog)s analyze owner/repo --config config.json
        """
    )
    ...
    if args.action == 'version':
        print("GitHub Repo Analyzer v1.1.0")
